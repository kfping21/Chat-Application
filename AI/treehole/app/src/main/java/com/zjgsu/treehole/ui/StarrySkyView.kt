...
class StarrySkyView @JvmOverloads constructor(
    ...
    // Enhanced orbs - spread out to avoid overlapping
    private val orbs = listOf(
        Orb(0.15f, 0.10f, 20f, 0xFF60A5FA.toInt(), 0x2060A5FA, 0f, 0.25f, 0.3f, mood = "最近好难过，家人的离世让我感到前所未有的孤独..."),
        Orb(0.50f, 0.08f, 18f, 0xFFFBBF24.toInt(), 0x20FBBF24, 1.2f, -0.3f, 0.2f, mood = "刚和最好的朋友吵了一架，心里特别难受..."),
        Orb(0.85f, 0.12f, 20f, 0xFF34D399.toInt(), 0x2034D399, 2.5f, 0.2f, 0.25f, mood = "深夜睡不着，突然很想找个人说说话..."),
        Orb(0.25f, 0.30f, 18f, 0xFFF472B6.toInt(), 0x20F472B6, 0.8f, -0.25f, 0.2f, mood = "工作压力好大，感觉自己快要撑不住了..."),
        Orb(0.65f, 0.28f, 20f, 0xFFA78BFA.toInt(), 0x20A78BFA, 3.1f, 0.25f, -0.2f, mood = "今天生日却一个人过，有点伤感..."),
        Orb(0.12f, 0.48f, 18f, 0xFF8B5CF6.toInt(), 0x208B5CF6, 1.7f, -0.2f, 0.25f, mood = "刚分手，心里空落落的，不知道该怎么度过..."),
        Orb(0.55f, 0.45f, 20f, 0xFFFBBF24.toInt(), 0x20FBBF24, 4.0f, 0.2f, -0.2f, mood = "考研失败了，不知道该何去何从..."),
        Orb(0.88f, 0.50f, 18f, 0xFFEC4899.toInt(), 0x20EC4899, 2.0f, 0.15f, 0.2f, mood = "一个人在外打拼，生病了只能自己照顾自己..."),
        Orb(0.40f, 0.58f, 16f, 0xFF34D399.toInt(), 0x2034D399, 3.5f, -0.2f, 0.15f, mood = "最近总是莫名感到焦虑...")
    )
        ...
        if (event.action == MotionEvent.ACTION_UP) {
            ...
            orbs.forEachIndexed { index, orb ->
                ...
                if (dist <= r * 1.6f) {
                    ...
                    // Vibrate feedback
                    performClick()
                    onOrbClickListener?.invoke(index, orb.mood)
                    return true
                }
            }
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        orbs.forEachIndexed { index, orb ->
            ...
            val floatY = cos(animProgress + orb.phase) * orb.speedY * 10f * density
            val cx = orb.cx * w + floatX
            // Limit Y position to 70% of height to avoid overlapping with text at bottom
            val cy = (orb.cy * h + floatY).coerceAtMost(h * 0.70f)
